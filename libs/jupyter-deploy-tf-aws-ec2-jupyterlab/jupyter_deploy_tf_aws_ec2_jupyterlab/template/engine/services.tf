...
locals {
  ...
  # role/user_allowlist re-seed /etc/AUTH_ALLOWLIST from these vars on every cloud-init run;
  # `jd users`/`jd teams` edit it at runtime (see update-allowlist.sh) and write changes back into
  # these vars, so the re-seed is a no-op rather than a revert.
...
locals {
  ...
  deployment_scripts_filenames = ["check-status-internal.sh", "get-status.sh", "update-server.sh", "generate-cert.sh", "update-allowlist.sh", "get-allowlist.sh"]
  ...
  all_script_files = {
    ...
    "deployment-scripts/update-allowlist.sh" = {
      content      = data.local_file.update_allowlist.content
      content_type = "text/x-shellscript"
    }
    "deployment-scripts/get-allowlist.sh" = {
      content      = data.local_file.get_allowlist.content
      content_type = "text/x-shellscript"
    }
...
DOC
  ...
  has_required_files = alltrue([
    ...
    fileexists("${path.module}/../services/commands/update-allowlist.sh"),
    fileexists("${path.module}/../services/commands/get-allowlist.sh"),
