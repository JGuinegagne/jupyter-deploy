...
locals {
  ...
  # role/user_allowlist seed /etc/AUTH_ALLOWLIST on FIRST BOOT ONLY; after that `jd users`/`jd
  # teams` own it at runtime (see update-allowlist.sh) and write changes back into these vars.
