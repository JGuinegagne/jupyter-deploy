...
- **EKS Cluster**: Managed Kubernetes control plane with (at least) three node pools:
  - **Platform**: A managed node group running control-plane services (JupyterK8s operator, Karpenter, Keda, etc.).
  - **Routing**: A Karpenter-provisioned pool running the routing tier (Traefik, Dex, Web UI).
  - **Workspaces**: One or more Karpenter-provisioned pools running user workspace pods with dedicated persistent volumes; these node pools scale to zero when idle.
- **Network Load Balancer**: Receives HTTPS traffic and forwards it to the Traefik ingress controller on the routing pool.
...
The platform layer handles routing, authentication, and workspace lifecycle. Control-plane
services run on the platform node group; the user-facing routing tier runs on the routing pool:
